...
#include <string.h>
#include <math.h>
//#include <malloc.h>        /* commented for MacOs */
...
{ /* semi-own free routine which makes additional test */
  if (p == NULL) error((char*) "freeing null");
  free(p);
}


static void *palloc(unsigned long size, long no)
{ /* semi-own alloc routine which makes additional test */
  long *p;
  size *= no;
  if (size == 0) size = 1;
  if (size != (size_t) size) error((char *) "Alloc too big %ld", size);
  p = (long *) malloc(size);
  if (p == NULL) error((char*) "no memory size %ld", size);
  return p;
}
...
static void push(allinfo *a, int side, item *f, item *l)
{
  ...
  if (a->intv1 == a->intv2) error((char*) "interval stack full");
  pos->f = f; pos->l = l;
}
...
static void pop(allinfo *a, int side, item **f, item **l)
{
  interval *pos;
  switch (side) {
    case LEFT : if (a->intv1 == a->intv1b) error((char*) "pop left");
                (a->intv1)--; pos = a->intv1;
                *f = pos->f; *l = pos->l;
                break;
    case RIGHT: if (a->intv2 == a->intv2b) error((char*) "pop right");
                (a->intv2)++; pos = a->intv2;
                *f = pos->f; *l = pos->l;
                break;
  }
...
static void improvesol(allinfo *a, state *v)
{
  ...
  if (v->wsum  > a->c) error((char*) "wrong improvesol");
  if (v->psum <= a->z) error((char*) "not improved solution");
...
static void definesolution(allinfo *a)
{
  ...
  if (f > l) error((char*) "wrong backtrack");
...
static void multiply(allinfo *a, item *h, int side)
{
  ...
  if ((k == i) || (k == j)) error((char*) "multiply, no space");
